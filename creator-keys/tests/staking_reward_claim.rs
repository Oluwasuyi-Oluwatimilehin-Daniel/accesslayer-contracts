...
use creator_keys::{events, StakeRewardClaim, StakingError, STAKE_LOCK_LEDGERS};
use soroban_sdk::{
...
fn test_claim_stake_reward_fails_with_no_stake() {
    ...
    let result = client.try_claim_stake_reward(&creator, &holder, &1u32);
...
fn test_claim_stake_reward_pays_out_and_unlocks_after_lock_period() {
    ...
    let claim: StakeRewardClaim = client.claim_stake_reward(&creator, &holder, &stake_id);
    assert_eq!(claim.reward, pool_after_buy);
    assert_eq!(claim.amount, 1);
    assert_eq!(claim.stake_id, stake_id);
...
fn test_claim_stake_reward_splits_pool_pro_rata_across_stakers() {
    ...
    let claim_a: StakeRewardClaim = client.claim_stake_reward(&creator, &holder_a, &stake_a);
    assert_eq!(claim_a.reward, expected_a);
    assert_eq!(client.get_total_staked(&creator), 1);

    let claim_b: StakeRewardClaim = client.claim_stake_reward(&creator, &holder_b, &stake_b);
